...
router.get('/',(req,res)=>{
    PostMessage.find((err,docs)=>{
        ...
        else console.log('Error while retrieveing all records : '+JSON.stringify(err,undefined,2))
    })
})
...
router.delete('/:id',(req,res)=>{
    ...
    PostMessage.findByIdAndRemove(req.params.id, (err,docs) => {
        if(!err) res.send(docs)
        else console.log('Error while deleting new record : ' + JSON.stringify(err,undefined,2))
    })
})
